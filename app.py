...
#
# 필요 파일:
#   T2_exit_demand_by_gate_dynamic.xlsx
# =========================================================
...
st.caption("동적 가중치 · 시간대별 교통수단 비율 · 출구별 변동성 반영")
st.markdown("---")


FILE_PATH = "T2_exit_demand_by_gate_dynamic.xlsx"
...
def load_data(path):
    ...
    return df, sheet_name, time_col, gate_col, people_col


try:
    df, sheet_name, time_col, gate_col, people_col = load_data(FILE_PATH)
...
def format_gate(x):
    n = extract_gate_num(x)
    if n == 999:
        return str(x)
    return f"{n}번"


df[gate_col] = df[gate_col].apply(format_gate)
...
gates = sorted(
    day_df[gate_col].unique(),
    key=extract_gate_num
)
...
pivot = pivot.reindex(
    sorted(pivot.index, key=extract_gate_num)
)
...
# ---------------------------------------------------------
st.markdown("---")
st.subheader("시간대별 추천 출구 변화")

recommend_rows = []

for t, group in day_df.groupby(time_col):
    g = group.sort_values(people_col)
    best = g.iloc[0]
    worst = g.iloc[-1]
    ...
    recommend_rows.append(
        {
            "시간": t,
            "추천 출구": best[gate_col],
            "추천 출구 인원": int(best[people_col]),
            "혼잡 출구": worst[gate_col],
            "혼잡 출구 인원": int(worst[people_col]),
        }
    )

recommend_df = pd.DataFrame(recommend_rows).sort_values("시간")
...
show_recommend_df = recommend_df.copy()
show_recommend_df["시간"] = show_recommend_df["시간"].dt.strftime("%Y-%m-%d %H:%M")
